...
class isotope(object) :
 ...
 def fCopyENDFdata(self):
  newisot=isotope()
  for sattr in ["z","a","liso","t12","elp","eem","ehp","lrtyp","lspectrum"]:
...
class lisotope(object) :
 ...
 def fGammaPrint(self,sfn="gamma.txt") :
  ...
  for time,lgamma in zip(self.lt,self.llgamma) :
   ...
   for tline in lgamma :
    f.write("%e %e %s \n" % (tline[0],tline[1], g.gcel.fGetOrigenName((tline[2],tline[3],tline[4]))))
   pass
  pass
  f.write("********************************\n")
  f.close()
 ...
 def fReadENDF(self,sdir=None,bVerbose=True) :
  ...
  if bVerbose:
   ...
   print("%s folder" % (sdir))
   print("Reading ENDF decay data files (. means read, x means stable isotope excluded)")
  pass 
  i=0
  j=0
  for sfn in lf :
   ...
   if ci.t12==0.0 :
    j+=1
    if bVerbose: sys.stdout.write("x") # zero half time - discarded isotope
   else :
    if bVerbose: sys.stdout.write(".") # included radioactive isotope
    self.fAdd(ci)
    i+=1
   if bVerbose: sys.stdout.flush()
  pass
  if bVerbose:
  ...
  for i,ci in enumerate(self.l) :
   ...
   if bMove :
    ...
    if k>=0 :
     ...
     pass
     if jj>=0 :
      if bVerbose : print(g.gcel.lmove[k],": ",j,g.gcel.lelmel[j]," -> ",jj,g.gcel.lelmel[jj])
      j=jj
     pass 
    pass               
   pass
 ...
 def fPower(self, elmass) :
  ...
  for ci,gi in zip(self.l,self.lg) : # ci is isotope, gi is melcor element index for this isotope
   iz = g.gcel.lelmelz[gi] # iz is Z of MELCOR element group gi, ci belongs to group gi
