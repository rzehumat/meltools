        ...
        for line in f:
            ...
            if mt == 457:
                ...
                if (
                    ...
                    if inl < 0 or (
                        inl == 0 and nt == 6
                    ):  # For gamma ray emission (STYP=0.0), no other information is required if X-ray, Auger electron, conversion electron, and pair formation intensities have not been calculated for these transitions. In this case NT=6.
                        iner -= 1
                        if iner == 0:
                            iner = -1
                        if int(lcon) == 0:
            ...
            for tline in lgamma:
                f.write(
                    ...
                    % (
                        tline[0],
                        tline[1],
                        gcel.fGetOrigenName((tline[2], tline[3], tline[4])),
                    )
                )
            pass
        pass
...
class config:
    ...
    def fDoIt(self):
        if len(self.lactifile) > 0:
            ...
            if len(self.lmassfile) > 0:
                ...
                if len(self.outpow) > 0:
                    self.lisot.fPrintPower(self.outpow)
                if len(self.outpowtot) > 0:
                    self.lisot.fPrintTotalPower(self.outpowtot, self.outpowtottimeunit)
                ...
                pass
            pass
            if len(self.outact) > 0:
                self.lisot.fPrintAct(self.loutisot, self.outact)
            if len(self.outactinp) > 0:
                self.lisot.fPrintActInp(self.loutisot, self.outactinp)
            if len(self.outactcsv) > 0:
                self.lisot.fPrintActCsv(self.loutisot, self.outactcsv)
    ...
    def fOutInv(self):
        ...
        print(
            (
                "Reference time is: %g s (=%g hours) (=%g days)"
                % (
                    self.ltime[0],
                    self.ltime[0] / 3600.0,
                    self.ltime[0] / (3600.0 * 24.0),
                )
            )
        )
        ...
        print(
            (
                "Writing initial inventory of element mass in kg to %s ..."
                % (self.soutinvmass)
            )
        )
    ...
    def fReadConf(self, sconf):
        ...
        for line in f:
            ...
            if ss[0] == "outpowtot":
                ...
                if len(ss) > 2:
                    self.outpowtottimeunit = ss[2].strip('"')
                continue
            pass
            if ss[0] == "outinc":
    ...
    def fPrintConf(self):
        ...
        for safile, xafrac, xatime, sunit in zip(
            self.lactifile, self.lactifrac, self.lactitime, self.lactiunit
        ):
        ...
        if len(self.outinc) > 0:
            print(" MELCOR DCH input %s for MELCOR %4.2f" % (self.outinc, self.xMelVer))
        if len(self.outact) > 0:
            print(" Activities in columns: %s" % self.outact)
        if len(self.outactinp) > 0:
            print(" Activities : %sxxxx.txt" % self.outactinp)
        if len(self.outgam) > 0:
            print(" Gamma lines: %s" % self.outgam)
        if len(self.outpow) > 0:
            print(" Power of MELCOR groups in colums: %s" % self.outpow)
        if len(self.outpowtot) > 0:
            print(" Total MELCOR power in colums: %s" % self.outpowtot)
        if self.bOutInv:
            print("Initial inventory output requested")
            print("         Activity in Bq: %s" % (self.soutinvacti))
            print(
                "         (sum of activity from fission and activation for each isotope with non zero activity)"
            )
            print(
                (
                    "        Reference time is: %g s (=%g hours) (=%g days)"
    ...
    def fReadActivities(self):
        ...
        lig = []
        cllisot = llisotope()
        for sfn, xfrac, xtime, sunit in zip(
            self.lactifile, self.lactifrac, self.lactitime, self.lactiunit
        ):
            clacti = lactivity()
            clacti.fRead(sfn)
...
gcel = elements()
# constants
xCiBq = 3.7e10  # Ci to Bq conversion factor
