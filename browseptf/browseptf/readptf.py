...
def fCheckGnuplot() :
 ...
 sgnuplot = "gnuplot"
 #
 p = subp.Popen([sgnuplot, "--version"], stdin=subp.PIPE,
...
def fTitle(sptf = "MELPTF") :
 ...
 p = subp.Popen(["readptf.exe", sptf, "list"], stdin=subp.PIPE,
                     stdout=subp.PIPE, stderr=subp.STDOUT)
 ss=""
 for line in p.stdout :
  ss = line.strip()
...
def fPlot(strplt = '\nplot cos(x)\n', lcp=0,sTitle="",sfT='fT(x)=x/3600.0\nset xlabel \"Time [h]\"\n') :
...
def fFormatVar(lVar,sptf = "MELPTF"):
 ...
 for sVarwi in lVar :
  ...
  pass
  if iOK>=0 :
   pltstr1 = '\"< readptf.exe %s %s%s%s \" using (fT($1)):2 title \"%s\" with lines' % (sptf,sap,sVar,sap,sVar)
  else :
   ss=sVar.split("#")
   sss=ss[0]
   iOK=-1
   ...
   if iOK>=0 :
    if len(sVarv)>3 :
     pltstr1 = '\"< readptf.exe %s %s%s%s \" using (fT($1)):%d title \"%s %s %s\" with lines' % (sptf,sap,sss,sdap,iv+1,sss,sVarv[1],sVarv[3])
    else :
     if len(sVarv)>1 :
      pltstr1 = '\"< readptf.exe %s %s%s%s \" using (fT($1)):%d title \"%s %s\" with lines' % (sptf,sap,sss,sdap,iv+1,sss,sVarv[1])
     else :
      pltstr1 = '\"< readptf.exe %s %s%s%s \" using (fT($1)):%d title \"%s#%d\" with lines' % (sptf,sap,sss,sdap,iv+1,sss,iv)
     pass
    pass 
   else :
    try :
    ...
    pass
    if bOK :
     pltstr1 = '\"< readptf.exe %s %s%s%s 2 \" using (fT($1)):2 title \"MAX %s\" with lines' % (sptf,sap,sss,sdap,sss)
     iOK=1
    else :
     try :
      bOK=(ss[-1]=="min")
     ...
     pass
     if bOK :
      pltstr1 = '\"< readptf.exe %s %s%s%s 2 \" using (fT($1)):3 title \"MIN %s\" with lines' % (sptf,sap,sss,sdap,sss)
      iOK=1
     else :
      try :
       bOK=(ss[-1]=="min0")
      ...
      pass
      if bOK :
       pltstr1 = '\"< readptf.exe %s %s%s%s 3 \" using (fT($1)):3 title \"(MIN>0) %s\" with lines' % (sptf,sap,sss,sdap,sss)
       iOK=1
      else :
       try :
        bOK=(ss[-1]=="sum")
       ...
       pass
       if bOK :
        pltstr1 = '\"< readptf.exe %s %s%s%s 4 \" using (fT($1)):2 title \"Sum %s\" with lines' % (sptf,sap,sss,sdap,sss)
        iOK=1
       pass
      pass 
     pass
...
def fInitLists() :
 ...
 lVarRN1Class = ["RN1-TOTMAS-1",
 "RN1-TOTMAS-2","RN1-TYCLT"] 
 ...
 lVarNCG = ["CVH-E","CVH-H","CVH-H","CVH-PPART","CVH-ECV","CVH-X","CVH-MASS","CVH-RHO", "FL-I-MFLOW", "FL-MFLOW"]
