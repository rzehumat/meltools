...
def fCheckPdfTk() :
 ...
 try :
  ...
  p = subp.Popen(["pdftk", "--version"], stdin=subp.PIPE,
...
def fCleanOutput(spdfdir,iClean=0) :
 ...
 if iClean == 1 :
  ...
  print("Checking number of files in the %s folder, checking pdf folder ..." % (spdfdir))   
...
def fCheckOutputRange(nt0,icstart,icstop,icstep,imaxname) :
 ...
 if nr==1 :
  ...
  if icstep>1 :
   print("Plot %d figures: %d ... %d, step %d" % (nr,icstart,nt0-1,icstep))
  else :
...
def fPlotScale(c,mcgr,x0,y0,x1,y1,tmax,tmin,ltp=[]) :
 ...
 y=y1 
 for i in range (iMax) :
  p = pyx.path.path(pyx.path.moveto(x0,y),\
 ...
 for i in lx :
  ...
  if tmin-dx/1000.0 <=  x <= tmax+dx/1000.0 :
   ...
   c.text(x1,y1-xi*dy,"\\,%d\\ " % (i),[ha,pyx.text.valign.middle,pts])   
  pass
 pass
 c.text((x0+x1)/2.0,y1+xtboxt,"\\textit{T} [K]",[pyx.text.halign.boxcenter,pyx.text.valign.bottom,pts])   
pass
...
def fPlotXaxis(c,zmin,zmax,lsxtics,dx,dz) :
 ...
 p = pyx.path.path(pyx.path.moveto(xmin,zmin-dz),pyx.path.lineto(xmax,zmin-dz))
 c.stroke(p)
 for sx in lsxtics :
  x = float(sx)
  p = pyx.path.path(pyx.path.moveto(x,zmin-dz),pyx.path.lineto(x,zmin-2*dz))
  c.stroke(p)
...
def fPlotCVHOneColor(c,mcgr,x00,lscv,lx0,lscvt,lscvz,lscvv,lscvx,ltvar,tmax,tmin) :
 ...
 for scv in lscv : 
  ...
  c.text(x0,(lz[-1]+lz[0])/2.0,scv,[pyx.text.halign.boxcenter,pyx.text.valign.middle])  
 pass
 return -1
pass
...
def fPlotCVH1TColor(c,mcgr,x0,lz,lx,tmax,tmin,t) :
 ii=len(lz)
 p = pyx.path.path(pyx.path.moveto(x0,lz[0]),
               pyx.path.lineto(x0+lx[0],lz[0])
               )
 for i in range(1,ii) :
...
def fPlotHSTemp(c,mcgr,x00,lhs,lhsx0,llhsx,llhst,lHSNumber,lHSba,lHSlal,tmax,tmin )  :
 ...
 for i in range (nfl) :
  ...
  if dzi < dz :
   ...
   try :
    ...
    iFrom = -1
   pass
   if iTo>=0 and iFrom>=0 : # only for flow paths connecting plotted cvh volumes
    xFrom = lx0[iFrom]
    xTo = lx0[iTo]
...
def  fPlotFLSimple(c,lfl,lscv,x00,xra,lx0,lflfrom,lflto,lflfromz,lfltoz, flcol = pyx.deco.color.grey.black ) :
 '''plot flow paths
    used in cvh-t.py, cvh-liqlev.py
 '''
 ifln = len(lfl) 
 for i  in range(ifln) :
  ...
  if (iFrom >= 0 and iTo >= 0) : 
   ...
   c.stroke(p,[pyx.deco.arrow(size=xra),pyx.deco.color.transparency(0.5), flcol ])
