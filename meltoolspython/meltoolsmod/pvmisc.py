...
def fReadVarIndex(sptf, index):
    ...
    l = []
    p = subp.Popen(
        ["readptf.exe", sptf, "varindex", "%s" % (index)],
        stdin=subp.PIPE,
        stdout=subp.PIPE,
        stderr=subp.STDOUT,
    )
...
def fSumVarsCol(lVars, liCol=[], sptf="MELPTF"):
    ...
    for i in range(len(lVars)):
        if type(liCol[i]) == list:
            ...
            pass
            lx1 = fReadVar(
                lVars[i], iMess=0, iCol=iCol - 1, sptf=sptf, sOpt="0"
            )  # do not include time column
        pass
        if len(lx1) == len(lx):
...
def fSumVars(lVars, sptf="MELPTF"):
    ...
    """
    llx = fReadVar(
        lsVar[0], iMess=0, iCol=-1, sptf=sptf, sOpt="1"
    )  # include time column
    for i in range(1, len(lsVar)):
        llx1 = fReadVar(
            lsVar[i], iMess=0, iCol=-1, sptf=sptf, sOpt="0"
        )  # do not include time column
