    ...
    for i in range(1, iMax):
        ...
        if it >= 0:
            c.text(
                ...
                y + dy,
                "\\," + ltp[it],
                [pyx.text.halign.boxleft, pyx.text.size.large],
            )
    pass
    c.text(
        x1,
        y1,
        "\\raisebox{1.2ex}{\\textit{T}\\,[K]}",
        [pyx.text.halign.boxleft, pyx.text.size.large],
    )
...
def fColorWall(
    ...
    if bPrint:
        ...
        print(
            "xHLST=%f,dzmax=%f,dzmax1=%f,z=%f,i0cw=%d" % (xHLST, dzmax, dzmax1, z, i0cw)
        )
        bPrint = False
    pass
    iii += 1
...
def fWallGrid(
    ...
    if nlh2 > 0:
        ...
        for i in range(nxs, iNLH):
            ...
            xfi = 180.0 * math.asin(rVessl / rLHl) / math.pi - 90.0
            p = pyx.path.path(
                pyx.path.moveto(x0, y0 - (drLHl + dzlh1)),
                pyx.path.arc(x0, y0 + xRVLH, rLHl, -90.0, xfi),
                pyx.path.lineto(rVessl, xHLST),
            )
    ...
    for xz in lz:
        if xz > lz[i0] and xz <= xHLST:
            x1 = xRVESS
            p = pyx.path.path(
                pyx.path.moveto(xRVESS, xz), pyx.path.lineto(xRVESS + dzrv1, xz)
            )
            c.stroke(p, [pyx.color.cmyk.Yellow])
            for i in range(0, nlh1 + 1):
            ...
            if nlh2 > 0:
                p = pyx.path.path(
                    pyx.path.moveto(xRVESS + dzrv1, xz),
                    pyx.path.lineto(xRVESS + dzrv1 + dzrv2, xz),
                )
                c.stroke(p, [pyx.color.cmyk.Green])
                nxs = nlh1 + 1
                for i in range(nxs, iNLH):
...
def fDBPoolColors(
    c, mcgr, it, l, lt, lvolf, nRings, nRows, ntlp, lr, lz, xRVLH, x0, y0
):
