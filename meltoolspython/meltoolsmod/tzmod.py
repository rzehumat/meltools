...
lline = [ True , True ,  True ,  True ,  True ,  
          True , True ,  True ,  True ,  True ,  
          True , True ,  True ,  True ,  True ]
lsymb = [ True , True ,  True ,  True ,  True ,  
          True , True ,  True ,  True ,  True ,  
          True , True ,  True ,  True ,  True ]
...
def fPlotTZ(lzc=[]) :
 ...
 if len(lzc)>0 :
  ...
  for i in range(len(lzc)-1) :
   lznodticks.append(pyx.graph.axis.tick.tick((lzc[i]+lzc[i+1])/2.0, label="%02d" % (i+1)))
  pass
  g = pyx.graph.graphxy(width=19,
       key=pyx.graph.key.key(pos='bc',hinside=0, vinside=0, vdist=2.0, columns=nColumns),
       x=pyx.graph.axis.linear(title=xtitle, 
         painter=my_axis_painter),
       y=pyx.graph.axis.linear(title=ytitle, 
         painter=my_axis_painter,min=lzc[0],max=lzc[-1]),
       y2=pyx.graph.axis.linear(title="Axial node [-] (at node center elevation)", 
         min=lzc[0],max=lzc[-1],
         manualticks=lznodticks,
         parter=None )
       ) 
 else :
  if zmin==zmax or (zmin==0.0 and zmax==10.0): fSetZMinMax()
  g = pyx.graph.graphxy(width=19,
       key=pyx.graph.key.key(pos='bc',hinside=0, vinside=0, vdist=2.0, columns=nColumns),
       x=pyx.graph.axis.linear(title=xtitle, 
         painter=my_axis_painter),
       y=pyx.graph.axis.linear(title=ytitle, 
         painter=my_axis_painter,min=zmin,max=zmax))
 ...
 for i in range (len(llz)) :
  ...
  pass
  if lline[ii] :
   st.append(pyx.graph.style.line([lcolors[ii],llinestyle[ii],llinewidth[ii]]))
  if lsymb[ii] :
   st.append(pyx.graph.style.symbol(lsymbols[ii],
                        size=lsymsize[ii],
...
def fWriteTimeTitle(g,xtime,itime,sTitle) :
  ...
  g.text(g.width/2,g.height+0.2,sTitle,[pyx.text.halign.center,pyx.text.valign.bottom])
pass
