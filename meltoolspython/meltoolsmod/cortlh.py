...
def fMain():
 ...
 for svar in ["xRVESS","xRVLH","dzlh2","dzlh1","dzrv2","dzrv1","nlh2","nlh1","ntlp"]:
 ...
 # read auxiliary data from the pickle file when necessary --- backward compatibility
  ...
  f.close()
  print(co.xRVESS,co.xRVLH,co.nlh2,co.nlh1,co.dzlh2,co.dzlh1,co.dzrv2,co.dzrv1,co.ntlp)
 pass
 nrad   = len(lr)
 xRCOR  = lr[nrad-2]
 ...
 yo1 = ( co.xRVLH - math.sqrt( (co.xRVLH+co.dzlh1)**2 - (co.xRVESS+co.dzrv1)**2 )) + y0
 xfio1=180.0*math.asin((co.xRVESS+co.dzrv1)/(co.xRVLH+co.dzlh1))/math.pi-90
 ...
 #colors in the debris bed/pool
  cortlhmod.fDBPoolColors(c,mcgr,it,l,lt,lvolf,nRings,nRows,co.ntlp,lr,lz,co.xRVLH,x0,y0) 
 
 # colors in the wall
  cortlhmod.fColorWall(c,mcgr,x0,y0,tanfi1,tanfi2,co.xRVESS,xDZRV,co.xRVLH,xDZLH,xHLST,co.nlh1,co.nlh2,lr,lz,xfii,xfio,yi,yo,co.dzlh1,co.dzlh2,co.dzrv1,co.dzrv2,ltime,l,it)
 ...
 # plot nodalization of the RPV wall
  cortlhmod.fWallGrid(c,lr,lz,co.xRVESS,co.xRVLH,xHLST,x0,y0,co.dzlh1,co.dzlh2,co.dzrv1,co.dzrv2,co.nlh1,co.nlh2,tanfi1,tanfi2)
  
  # plot titles
  x=ltime[it]
  xtboxt = pyx.text.text(0.0, 0.0, "{\\tiny X}").bbox().height()
  ss = "\\ \\\\Time = %.1f\,s = %s, Plot record %d" % (x, pvmisc.fCas(x),it)  
  c.text(x0,lz[0]-(xtboxt+xDZLH),ss,
         [pyx.text.halign.boxleft,pyx.text.valign.top,tts])
  c.text(x0,xHLST+xtboxt,"\\noindent "+co.sTitle+"\\\\ \\,",
