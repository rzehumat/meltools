...
def fMain():
 ...
 lscvt,lscvz,lscvv,lfl,lflfrom,lflto,lflfromz,lfltoz = pvmisc.fGetCVFLSetup(co.sptf)
 ...
 pass
 #
 if len(co.lHSlal)>0 : lHSlal=co.lHSlal
 ...
 for iRec in ltr :
  ...
  #print (tmin,tmax)
  pvpyx.fPlotScale(c,mcgr,co.x0tscale,zmin1,co.x0tscale+co.dxtscale,zmax1,tmax,tmin,co.lttics) 
  ...
  if len(co.shslist)>0 :
   #rint(tmax, tmin, iRec)
   #rint(lHSlal)
   pvpyx.fPlotHSTemp(c, mcgr, co.x00, lhs, lhsx0, llhsx, lhst[iRec], lHSNumber, lHSba, lHSlal, tmax, tmin ) 
  pass
  sf="./%s/%04d" % (co.spdfdir,iRec)
  sz = "Time = %.1f\,s = %s, Plot record %d" % (ltime[iRec], pvmisc.fCas(ltime[iRec]),iRec)  
